...
def log_save(sender, instance, created, **kwargs):
    ...
    ActivityLog.objects.create(message=f"Программа '{instance}' была {verb}.")


@receiver(post_delete, sender=Program)
def log_delete(sender, instance, **kwargs):
    ActivityLog.objects.create(message=f"Программа '{instance}' была удаленна.")
...
def log_save(sender, instance, created, **kwargs):
    verb = "создан" if created else "обновлен"
    ActivityLog.objects.create(message=f"Курс '{instance}' был {verb}.")


@receiver(post_delete, sender=Course)
def log_delete(sender, instance, **kwargs):
    ActivityLog.objects.create(message=f"Курс '{instance}' был удален.")
...
class Upload(models.Model):
    ...
    file = models.FileField(
        ...
        help_text="Допустимы форматы: pdf, docx, doc, xls, xlsx, ppt, pptx, zip, rar, 7zip",
        validators=[
...
def log_save(sender, instance, created, **kwargs):
    if created:
        ActivityLog.objects.create(
            message=f"Файл '{instance.title}' добавлен в курс '{instance.course}'."
        )
    else:
        ActivityLog.objects.create(
            message=f"Файл '{instance.title}' из курса '{instance.course}' был обновлен."
        )
...
def log_delete(sender, instance, **kwargs):
    ActivityLog.objects.create(
        message=f"Файл '{instance.title}' из курса '{instance.course}' был удален."
    )
...
class UploadVideo(models.Model):
    ...
    video = models.FileField(
        ...
        help_text="Допустимы форматы: mp4, mkv, wmv, 3gp, f4v, avi, mp3",
        validators=[
...
def log_save(sender, instance, created, **kwargs):
    if created:
        ActivityLog.objects.create(
            message=f"Видео '{instance.title}' было добавлено в курс {instance.course}."
        )
    else:
        ActivityLog.objects.create(
            message=f"Видео '{instance.title}' из курса '{instance.course}' было обновленно."
        )
...
def log_delete(sender, instance, **kwargs):
    ActivityLog.objects.create(
        message=f"Видео '{instance.title}' из курса '{instance.course}' было удаленно."
    )


class CourseOffer(models.Model):
    """NOTE: Только заведующий кафедрой может предложить курсы на семестр."""
