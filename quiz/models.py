...
class Quiz(models.Model):
    ...
    exam_paper = models.BooleanField(
        blank=False,
        default=False,
        verbose_name=_("Экзаменационная работа"),
        help_text=_(
            "Если да, результат каждой попытки пользователя будет сохранен. Необходимо для оценки."
        ),
    )
    ...
    pass_mark = models.SmallIntegerField(
        blank=True,
        default=50,
        verbose_name=_("Процент для прохождения"),
        validators=[MaxValueValidator(100)],
        help_text=_("Процент, необходимый для прохождения экзамена."),
    )
    ...
    draft = models.BooleanField(
        ...
        help_text=_(
            "Если да, то тест не отображается в списке тестов и может быть пройден только пользователями, которые могут редактировать тесты."
        ),
    )
    ...
    def save(self, force_insert=False, force_update=False, *args, **kwargs):
        ...
        if self.pass_mark > 100:
            raise ValidationError("%s превышает 100" % self.pass_mark)
        if self.pass_mark < 0:
            raise ValidationError("%s ниже 0" % self.pass_mark)
...
class Progress(models.Model):
    ...
    class Meta:
        ...
        verbose_name_plural = _("Запись успеваемости пользователя")
    ...
    def update_score(self, question, score_to_add=0, possible_to_add=0):
        ...
        if any(
            ...
            return _("error"), _("Категория не существует или оценка недействительна.")
    ...
    def new_sitting(self, user, quiz, course):
        ...
        if len(question_set) == 0:
            raise ImproperlyConfigured(
                "Вопросы для теста не заданы. Пожалуйста, настройте вопросы корректно."
            )
...
class Sitting(models.Model):
    ...
    current_score = models.IntegerField(verbose_name=_("Текущая оценка"))
    ...
    start = models.DateTimeField(auto_now_add=True, verbose_name=_("Начало"))
    end = models.DateTimeField(null=True, blank=True, verbose_name=_("Конец"))
    ...
    def result_message(self):
        if self.check_if_passed:
            return f"Вы успешно прошли этот тест, поздравляем!"
        else:
            return f"Вы не сдали этот тест. Попробуйте в следующий раз."
...
class Question(models.Model):
    ...
    figure = models.ImageField(
        ...
        blank=True,
        null=True,
       verbose_name=_("Рисунок"),
        help_text=_("Добавьте изображение для вопроса, если это необходимо."),
    )
...
class MCQuestion(Question):
    choice_order = models.CharField(
        ...
        help_text=_(
            "Порядок, в котором варианты выбора множественного выбора отображаются пользователю"
        ),
...
class Choice(models.Model):
    ...
    choice = models.CharField(
        max_length=1000,
        blank=False,
        help_text=("Введите текст выбора, который вы хотите отобразить"),
