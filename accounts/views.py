...
def edit_staff(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "Данные учителя " + full_name + " были обновлены.")
            return redirect("lecturer_list")
        else:
            messages.error(request, "Ведденые данные не прошли валидацию.")
    else:
...
def delete_staff(request, pk):
    ...
    lecturer.delete()
    messages.success(request, "Профиль учителя " + full_name + " был удален.")
    return redirect("lecturer_list")
...
def student_add_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "Аккаунт " + first_name + " " + last_name + " был создан.",
            )
            return redirect("student_list")
        else:
            messages.error(request, "Ошибка при валидации данных.")
    else:
        form = StudentAddForm()
...
def edit_student(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(request, "Ошибки при валидации данных.")
    else:
        form = ProfileUpdateForm(instance=instance)
    return render(
        request,
...
def render_student_pdf_list(request):
    ...
    # if error then show some funny view
    if pisa_status.err:
       return HttpResponse('Неожиданная ошибка <pre>' + html + '</pre>')
    return response
...
def delete_student(request, pk):
    ...
    student.delete()
    messages.success(request, "Профиль студента был удален.")
    return redirect("student_list")
